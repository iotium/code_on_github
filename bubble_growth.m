...
while running == 1;
    ...
    V_l_star = V_l(n+1)/(1 - V_bubi);
    
    
    % fill level based on liquid and tank volume
    fill_level(n+1) = V_l_star/V_tank;
...
for i = 1:N*2
    ...
    death_int(i) = sum( r_q.^(i-1) .* w_q .* C_death_rate ...
...
while ill_conditioned
    ...
    r_qp = r_q + (0.5 - rand(size(r_q))).*abs(r_q)*1e-3;
...
mdot_bub_l = V_l_star * 4/3*pi * rho_tg_sat * (birth_int(4) + growth_int(4));

% mdot into bubbles from tg
mdot_bub_tg = - V_l_star * 4/3*pi * rho_tg_sat * (death_int(4));
