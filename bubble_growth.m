...
if nargin == 0
    ...
    if isa(varargin{1}, 'char')
        constants.C_rdot = str2num( varargin{1} );
        constants.C_nuc_rate = str2num( varargin{2} );
        constants.C_death_rate = str2num( varargin{3} );
        constants.C_erf_death_rate = str2num( varargin{4} );
    else
    ...
    constants.coalescence_switch = 'on';
    constants.r_death = 0.5 * varargin{5} * 0.0254;
    specified_case = 6;
    save_filename = varargin{6};
    A_inj = 6.977e-7;
    E = 7.5e2;
end
...
if strcmp(constants.coalescence_switch,'on')
    ...
    Q = V_tank/8; % volumetric gas flow rate. just assume a constant value here
